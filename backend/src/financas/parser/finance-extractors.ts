...
export const ANCORAS_VALOR_COBRANCA: RegExp[] = [
  wb('\\bvalor a pagar\\b'),
  wb('\\btotal a pagar\\b'),
  wb('\\btotal da fatura\\b'),
  wb('\\bvalor da fatura\\b'),
  wb('\\bvalor do boleto\\b'),
  wb('\\bvalor da conta\\b'),
  wb('\\btotal da conta\\b'),
];
...
const TODAS_ANCORAS_VALOR = [
  ...ANCORAS_VALOR_COBRANCA,
  ...ANCORAS_VALOR_EXTRACAO,
];
...
const MOEDA_OPCIONAL_RE = new RegExp(
  String.raw`(?:R\$\s*)?(?<![\d,.])${MOEDA_NUM}(?![\d%])`,
);
...
export function extrairValor(texto: string): {
  valor: number | null;
  anchored: boolean;
  ancoraCobranca: boolean;
} {
  ...
  for (const ancora of TODAS_ANCORAS_VALOR) {
    ...
    const global = new RegExp(
      ancora.source,
      ancora.flags.includes('g') ? ancora.flags : `${ancora.flags}g`,
    );
    for (let i = 0; i < linhas.length; i++) {
      ...
      for (const m of matches) {
        ...
        if (APOS_MINIMO_INICIO_RE.test(antesMoeda)) {
          moedaRejeitadaNaLinha = true;
          continue;
        }
        if (
          PARCELAMENTO_RE.test(
            depois.slice(0, primeiraMoeda.index + primeiraMoeda[0].length),
          )
        ) {
          moedaRejeitadaNaLinha = true;
          continue;
        ...
        return {
          valor: zeroParaNull(parseBr(primeiraMoeda[1])),
          anchored: true,
          ancoraCobranca: ehCobranca,
        };
      }
  ...
  if (todas.length === 1) {
    ...
    if (!PARCELAMENTO_RE.test(linhaDoMatch)) {
      return {
        valor: zeroParaNull(parseBr(todas[0][1])),
        anchored: false,
...
const CONECTOR_RE =
  /^\s*(?:[:.\-–—]+\s*)?(?:(?:em|no dia|dia|para o dia)\s+)?\s*/i;
const MESES = [
  'janeiro',
  'fevereiro',
  'março',
...
const DATA_COMPLETA_RE =
  /(?<!\d)(\d{1,2})([./-])(\d{1,2})\2(\d{4}|\d{2})(?!\d)/;
const DATA_ISO_RE = /(\d{4})-(\d{2})-(\d{2})/;
const DATA_EXTENSO_RE = new RegExp(
...
export function inferirAno(
  ...
  return candidatos.reduce((melhor, d) =>
    Math.abs(d.getTime() - recebidoEm.getTime()) <
    Math.abs(melhor.getTime() - recebidoEm.getTime())
      ? d
      : melhor,
  );
}
...
export function proximoDia(dia: number, recebidoEm: Date): Date | null {
  ...
  const y = recebidoEm.getUTCFullYear(),
    m = recebidoEm.getUTCMonth() + 1;
...
function dataCompleta(texto: string): Date | null {
  ...
  if (n) {
    const yyyy = n[4].length === 2 ? 2000 + Number(n[4]) : Number(n[4]);
    return dataValida(yyyy, Number(n[3]), Number(n[1]));
  }
  const ext = texto.match(DATA_EXTENSO_RE);
  if (ext && ext[3])
    return dataValida(
      Number(ext[3]),
      MESES.indexOf(ext[2].toLowerCase()) + 1,
      Number(ext[1]),
    );
  return null;
}
...
function dataAposAncora(depois: string, recebidoEm: Date): Date | null {
  ...
  if (ddmm && semConector.indexOf(ddmm[0]) < 40)
    return inferirAno(Number(ddmm[1]), Number(ddmm[2]), recebidoEm);
...
export function extrairDataVencimento(
  texto: string,
  recebidoEm: Date,
): { data: Date | null; anchored: boolean } {
  const linhas = texto.split(/\r?\n/);
  for (const ancora of DATE_ANCHORS) {
    for (let i = 0; i < linhas.length; i++) {
      ...
      if (
        proxima !== undefined &&
        depois.replace(CONECTOR_RE, '').trim() === ''
      ) {
  ...
  return fallback
    ? { data: fallback, anchored: false }
    : { data: null, anchored: false };
}
...
export function extrairCodigoBarras(texto: string): string | null {
  return (
    texto.match(BOLETO_47_RE)?.[0] ??
    texto.match(CONCESSIONARIA_48_RE)?.[0] ??
    null
  );
}
